...
const text = (value, x, y, size = 10, bold = false) => `BT /F${bold ? 2 : 1} ${size} Tf ${x} ${y} Td (${escapePdfText(value)}) Tj ET`;
...
const buildPdf = (record) => {
    const commands = [
        ...
        text(`Report Date: ${record.createdAt ? new Date(record.createdAt).toLocaleString() : 'Not recorded'}`, 300, 665, 10),
    ];
    let y = 625;
    [['Chief Complaint', record.chiefComplaint], ['Diagnosis', record.diagnosis], ['Consultation Notes', record.consultationNotes]].forEach(([label, value]) => {
        commands.push(text(label, 55, y, 11, true));
        y -= 18;
        wrap(value).slice(0, 12).forEach((line) => { commands.push(text(line, 65, y)); y -= 15; });
        y -= 12;
    });
    if (record.followUpDate) commands.push(text(`Follow-up Date: ${new Date(record.followUpDate).toLocaleDateString()}`, 55, y, 10, true));
