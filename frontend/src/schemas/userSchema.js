...
export const userRoles = ['admin', 'doctor', 'nurse', 'pharmacist', 'receptionist', 'lab_technician', 'radiologist', 'patient'];
...
export const commonUserSchema = z.object({
    firstName: requiredText('First name').min(2, 'First name must be at least 2 characters').regex(personNameRegex, 'First name contains invalid characters'),
    lastName: requiredText('Last name').min(2, 'Last name must be at least 2 characters').regex(personNameRegex, 'Last name contains invalid characters'),
    email: requiredText('Email address').email('Enter a valid email address'),
    phone: requiredText('Phone number').regex(sriLankanPhoneRegex, 'Phone number must be a valid Sri Lankan 10-digit number starting with 0.'),
    nic: requiredText('NIC').regex(sriLankanNicRegex, 'NIC must be 12 digits or 9 digits followed by V/X.').transform((value) => value.toUpperCase()),
