        ...
        <main className="grid min-h-screen place-items-center bg-slate-50 p-5">
            <Card className="w-full max-w-md shadow-lg shadow-slate-200/60">
                <CardHeader>
                    ...
                    <CardDescription>
                        Recovery option using their registered mobile number.
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    {error && (
                    ...
                    {step === 'reset' && developmentOtp && (
                        <div className="mb-5 rounded-lg border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950">
                            ...
                            <p className="mt-1">
                                Your offline demonstration OTP is{' '}
                                <strong className="font-mono text-lg">{developmentOtp}</strong>.
                            </p>
                            <p className="mt-1 text-xs text-amber-800">
                                This preview is available only when explicitly enabled outside
                                production.
                            </p>
                            <Button
                                className="mt-3"
                    ...
                    {step === 'request' && (
                        <form onSubmit={requestOtp} className="space-y-4">
                            <Field icon={Phone} label="Registered mobile number">
                                <Input
                                    id="phone"
                                    className="pl-9"
                                    value={phone}
                    ...
                    {step === 'reset' && (
                        <form onSubmit={resetPassword} className="space-y-4">
                            ...
                            <button
                                className="w-full text-sm font-semibold text-cyan-700 hover:underline disabled:opacity-50"
                                type="button"
                                onClick={resendOtp}
                                disabled={submitting}
                            >
