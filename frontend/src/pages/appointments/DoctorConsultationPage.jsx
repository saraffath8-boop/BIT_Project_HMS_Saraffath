...
export default function DoctorConsultationPage() {
    ...
    const updatePrescriptionItem = (index, event) =>
        setPrescription({
            ...prescription,
            items: prescription.items.map((item, itemIndex) =>
    ...
    const submit = async (event) => {
        ...
        if (includeLab)
            payload.labRequest = {
                ...
                tests: labRequest.testNames
                    .split(',')
                    .map((testName) => ({ testName: testName.trim() }))
        ...
        try {
            ...
            setSuccess(
                `${response.message}. The diagnosis report was saved and created requests were routed to their relevant sections.`,
            );
    ...
    if (loading)
        return (
            <main>
                <Card className="p-6 text-sm text-slate-500">Loading appointment...</Card>
            </main>
        );
    ...
    return (
        <main className="space-y-6">
            <section className="flex flex-wrap items-start justify-between gap-4">
                <div>
                    <p className="page-kicker">Doctor Queue</p>
                    <h1 className="page-title">
                        {appointment?.patient?.fullName || 'Patient Diagnosis Report'}
                    </h1>
                    <p className="page-description">
                        {appointment
                            ? `${new Date(appointment.appointmentDate).toLocaleString()} · ${appointment.department} · Status: ${appointment.status.replaceAll('_', ' ')}`
                            : 'Open a checked queue entry and record the diagnosis report.'}
                    </p>
                </div>
            ...
            <form onSubmit={submit} className="space-y-6">
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Required consultation documentation linked to this appointment.
                        </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        ...
                        <Field label="Consultation Notes">
                            <Textarea
                                ...
                                value={medicalRecord.consultationNotes}
                                onChange={updateRecord}
                            />
                        </Field>
                        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            {Object.keys(emptyVitals).map((field) => (
                                <Field key={field} label={field.replace(/([A-Z])/g, ' $1')}>
                                    <Input
                                        name={field}
                                        value={medicalRecord.vitalSigns[field]}
                ...
                <OptionalCard
                    ...
                    onChange={setIncludePrescription}
                    icon={Pill}
                    title="Prescription"
                    description="The cashier receives this prescription first; payment sends it to the pharmacist."
                >
                    {prescription.items.map((item, index) => (
                        ...
                        >
                            <div className="grid gap-4 sm:grid-cols-2">
                                {Object.keys(emptyPrescriptionItem).map((field) => (
                                    <Field key={field} label={field.replace(/([A-Z])/g, ' $1')}>
                                        <Input
                                            name={field}
                                            value={item[field]}
                                            onChange={(event) =>
                                                updatePrescriptionItem(index, event)
                                            }
                                            required={
                                                includePrescription && field !== 'instructions'
                                            }
                                        />
                                    </Field>
                                ))}
                    ...
                    <Field label="Prescription Notes">
                        <Textarea
                            ...
                            onChange={(event) =>
                                setPrescription({ ...prescription, notes: event.target.value })
                            }
                        />
                    </Field>
                </OptionalCard>
                ...
                <OptionalCard
                    checked={includeLab}
                    onChange={setIncludeLab}
                    icon={FlaskConical}
                    title="Laboratory Request"
                    description="Comma-separated tests awaiting the patient's decision."
                >
                    <Field label="Test Names">
                        <Input
                            value={labRequest.testNames}
                            onChange={(event) =>
                                setLabRequest({ ...labRequest, testNames: event.target.value })
                            }
                            placeholder="Full blood count, Blood glucose"
                            required={includeLab}
                        />
                    </Field>
                    <Field label="Priority">
                        <select
                            ...
                            onChange={(event) =>
                                setLabRequest({ ...labRequest, priority: event.target.value })
                            }
                        >
                ...
                <OptionalCard
                    ...
                    onChange={setIncludeRadiology}
                    icon={ScanLine}
                    title="Radiology Request"
                    description="Imaging request awaiting the patient's decision."
                >
                    <div className="grid gap-4 sm:grid-cols-2">
                        <Field label="Scan Type">
                            <Input
                                value={radiologyRequest.scanType}
                                onChange={(event) =>
                                    setRadiologyRequest({
                                        ...radiologyRequest,
                                ...
                                required={includeRadiology}
                            />
                        </Field>
                        <Field label="Body Part">
                            <Input
                                value={radiologyRequest.bodyPart}
                                onChange={(event) =>
                                    setRadiologyRequest({
                                        ...radiologyRequest,
                    ...
                    <Field label="Clinical Reason">
                        <Textarea
                            value={radiologyRequest.clinicalReason}
                            onChange={(event) =>
                                setRadiologyRequest({
                                    ...radiologyRequest,
                                    clinicalReason: event.target.value,
                                })
                            }
                        />
                    </Field>
                ...
                <Button type="submit" size="lg" disabled={submitting || Boolean(success)}>
                    {submitting
                        ? 'Saving diagnosis report...'
                        : 'Save Diagnosis Report and Requests'}
                </Button>
            </form>
        </main>
    );
    ...
    <Card>
        <CardHeader>
            <label className="flex cursor-pointer items-start gap-3">
                ...
                />
                <Icon className="mt-0.5 size-5 text-cyan-700" />
                <span>
        ...
        {checked && <CardContent className="space-y-4">{children}</CardContent>}
    </Card>
);
