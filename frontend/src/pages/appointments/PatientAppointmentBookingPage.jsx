...
export default function PatientAppointmentBookingPage() {
    ...
    const isPatient = typeof user?.role === 'string' && user.role.trim().toLowerCase() === 'patient';
    ...
    return (
        <main className="min-h-screen bg-slate-50">
            ...
            <div className="mx-auto max-w-7xl space-y-6 px-5 py-8 sm:px-8">
                ...
                {!isPatient && <Alert>No patient account is required. Enter your details and submit the request; it will appear in the receptionist&apos;s pending appointments.</Alert>}
                ...
                <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1.35fr)_minmax(320px,.65fr)]">
                    ...
                    <Card className="lg:sticky lg:top-6"><CardHeader><CardTitle>Appointment Request</CardTitle><CardDescription>Review your selection before submitting.</CardDescription></CardHeader><CardContent>
                        <form onSubmit={submit} className="space-y-5">
                            ...
                            {!isPatient && <div className="space-y-4 rounded-xl border border-slate-200 p-4">
                                <div><p className="font-semibold text-slate-950">Patient Details</p><p className="mt-1 text-xs text-slate-500">These details identify the appointment request for the receptionist.</p></div>
                                <div><Label htmlFor="fullName">Full Name</Label><Input id="fullName" required maxLength={120} value={patientDetails.fullName} onChange={(event) => setPatientDetails({ ...patientDetails, fullName: event.target.value })} className="mt-2" /></div>
                                <div><Label htmlFor="phone">Phone Number</Label><Input id="phone" required inputMode="numeric" maxLength={10} placeholder="07XXXXXXXX" value={patientDetails.phone} onChange={(event) => setPatientDetails({ ...patientDetails, phone: event.target.value })} className="mt-2" /></div>
                                <div><Label htmlFor="dateOfBirth">Date of Birth</Label><Input id="dateOfBirth" required type="date" max={new Date().toISOString().slice(0, 10)} value={patientDetails.dateOfBirth} onChange={(event) => setPatientDetails({ ...patientDetails, dateOfBirth: event.target.value })} className="mt-2" /></div>
                                <div><Label htmlFor="gender">Gender</Label><select id="gender" required className="mt-2 h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm" value={patientDetails.gender} onChange={(event) => setPatientDetails({ ...patientDetails, gender: event.target.value })}><option value="">Select gender</option><option value="male">Male</option><option value="female">Female</option><option value="other">Other</option></select></div>
                                <div><Label htmlFor="address">Address</Label><Input id="address" required maxLength={300} value={patientDetails.address} onChange={(event) => setPatientDetails({ ...patientDetails, address: event.target.value })} className="mt-2" /></div>
                                <div><Label htmlFor="emergencyContactName">Emergency Contact Name</Label><Input id="emergencyContactName" required maxLength={120} value={patientDetails.emergencyContactName} onChange={(event) => setPatientDetails({ ...patientDetails, emergencyContactName: event.target.value })} className="mt-2" /></div>
                                <div><Label htmlFor="emergencyContactPhone">Emergency Contact Phone</Label><Input id="emergencyContactPhone" required inputMode="numeric" maxLength={10} placeholder="07XXXXXXXX" value={patientDetails.emergencyContactPhone} onChange={(event) => setPatientDetails({ ...patientDetails, emergencyContactPhone: event.target.value })} className="mt-2" /></div>
                            </div>}
                            <div><Label htmlFor="reason">Reason for Visit</Label><textarea id="reason" maxLength={500} rows={5} value={reason} onChange={(event) => setReason(event.target.value)} disabled={!timeSlot} className="mt-2 w-full rounded-lg border border-slate-200 bg-white p-3 text-sm disabled:bg-slate-100 disabled:text-slate-400" placeholder={timeSlot ? 'Briefly describe the reason for your visit' : 'Select a date and time slot first'} /></div>
