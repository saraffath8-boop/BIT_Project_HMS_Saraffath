// This file contains the radiology request create page interface.
...
// Show the radiology request create page interface.
const RadiologyRequestCreatePage = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        const payload = {
            ...
            bodyPart: getOptionalValue(formData.bodyPart),
            clinicalReason: getOptionalValue(formData.clinicalReason),
            scheduledAt: getOptionalValue(formData.scheduledAt),
        };
        if (user?.role !== 'doctor') payload.doctor = formData.doctor;
        try {
            await createRadiologyRequest(payload, token);
            setSuccess('Radiology request created successfully.');
            navigate('/radiology');
        } catch (err) {
            setError(err.message || 'Unable to create radiology request');
        } finally {
            setSubmitting(false);
        }
    };
    return (
        <main style={styles.page}>
            <section style={styles.header}>
                <div>
                    <p style={styles.kicker}>Radiology Management</p>
                    <h1 style={styles.title}>Create Radiology Request</h1>
                    <p style={styles.subtitle}>
                        Request imaging for a selected patient with scan and scheduling details.
                    </p>
                </div>
                <div style={styles.actions}>
                    <Link style={styles.secondaryLink} to="/radiology">
                        Radiology
                    </Link>
            ...
            <form style={styles.form} onSubmit={handleSubmit}>
                ...
                <button style={styles.primaryButton} type="submit" disabled={submitting}>
                    {submitting ? 'Creating Radiology Request...' : 'Create Radiology Request'}
                </button>
            </form>
        </main>
    );
