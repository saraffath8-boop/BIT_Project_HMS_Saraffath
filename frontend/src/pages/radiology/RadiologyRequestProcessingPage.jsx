...
export default function RadiologyRequestProcessingPage() {
        ...
        getRadiologyRequestById(id, token)
            .then(({ radiologyRequest }) => {
                ...
                setForm({
                    ...
                    scheduledAt: radiologyRequest.scheduledAt
                        ? new Date(radiologyRequest.scheduledAt).toISOString().slice(0, 16)
                        : '',
        ...
        try {
            ...
            setSuccess(
                form.status === 'completed'
                    ...
                    : 'Radiology request updated successfully.',
            );
        } catch (err) {
    ...
    if (loading)
        return (
            <main>
                <Card className="p-6 text-sm text-slate-500">Loading radiology request...</Card>
            </main>
        );
    return (
        <main className="space-y-6">
            <section className="flex flex-wrap items-start justify-between gap-4">
                <div>
                    <p className="page-kicker">Radiology Processing</p>
                    <h1 className="page-title">
                        {request?.patient?.fullName || 'Radiology Request'}
                    </h1>
                    <p className="page-description">
                        {request?.scanType} | {request?.bodyPart || 'Body part not recorded'}
                    </p>
                </div>
                <Button asChild variant="outline">
                    <Link to="/radiology">Radiology Requests</Link>
                </Button>
            </section>
                ...
                <form onSubmit={save} className="space-y-6">
                    <Card>
                        <CardHeader>
                            <div className="flex flex-wrap justify-between gap-3">
                                <div>
                                    ...
                                    <CardDescription>
                                        Clinical reason: {request.clinicalReason || 'Not recorded'}
                                    </CardDescription>
                                </div>
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <Field label="Scan/Image URL">
                                <Input
                                    disabled={!canProcess}
                                    name="imageUrl"
                                    value={form.imageUrl}
                                    onChange={change}
                                    placeholder="Secure uploaded scan URL"
                                />
                            </Field>
                    ...
                    {canProcess && (
                        <Button type="submit" disabled={saving}>
                            {saving ? 'Saving report...' : 'Save Radiology Report'}
                        </Button>
                    )}
                </form>
            )}
