...
const RadiologyPage = () => {
    ...
    return <ModuleListPage
        ...
        createAction={{ to: '/radiology/new', label: 'Add Radiology Request', allowedRoles: ['admin'] }}
        ...
        columns={[
            ...
            { label: 'Report', allowedRoles: ['admin', 'doctor', 'radiologist'], render: (item) => user?.role === 'radiologist' && item.paymentStatus !== 'paid' && item.status !== 'completed' ? 'Awaiting Payment' : <Link className="font-semibold text-cyan-700" to={`/radiology/${item.id}/process`}>{item.status === 'completed' ? 'View Report' : 'Process'}</Link> },
        ]}
    />
};
