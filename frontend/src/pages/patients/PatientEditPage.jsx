...
import { bloodGroupOptions, normalizePatientFormData, patientGenderOptions, patientSchema, patientStatusOptions } from '../../schemas/patientSchema';
...
export default function PatientEditPage() {
    ...
    useEffect(() => {
        const loadPatient = async () => {
            try {
                ...
                reset({ ...defaults, ...normalizePatientFormData(patient), dateOfBirth: toDateInputValue(patient.dateOfBirth) });
    ...
    return <main style={styles.page}><section style={styles.header}><div><p style={styles.kicker}>Patient Management</p><h1 style={styles.title}>Edit Patient</h1><p style={styles.subtitle}>{isNurse ? 'Nurses can update contact, status, allergies, and notes.' : 'Update patient demographic and clinical summary fields.'}</p></div><Link style={styles.secondaryLink} to={`/patients/${id}`}>Back to Profile</Link></section>
        ...
        <form onSubmit={handleSubmit(submit)} style={styles.formCard}><div style={styles.grid}>
            ...
            <Field label="Date of Birth" error={errors.dateOfBirth?.message}><input style={styles.input} type="date" readOnly={isNurse} {...register('dateOfBirth')} /></Field>
            <Field label="Gender" error={errors.gender?.message}><Select aria-disabled={isNurse} style={{ ...styles.input, pointerEvents: isNurse ? 'none' : 'auto' }} {...register('gender')}>{patientGenderOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</Select></Field>
...
const Field = ({ label, error, children }) => <label style={styles.label}>{label}{children}{error && <span style={styles.fieldError}>{error}</span>}</label>;
const styles = {
    page: { color: '#0f172a' }, header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '18px', background: '#0f172a', color: '#fff', padding: '28px', borderRadius: '24px', marginBottom: '20px' },
    kicker: { margin: 0, color: '#0e7490', fontWeight: 800, textTransform: 'uppercase' }, title: { margin: '8px 0', fontSize: '34px' }, subtitle: { margin: 0, color: '#64748b' }, secondaryLink: { background: '#e2e8f0', color: '#0f172a', textDecoration: 'none', padding: '12px 16px', borderRadius: '12px', fontWeight: 800 },
    card: { background: '#fff', borderRadius: '18px', padding: '24px' }, formCard: { background: '#fff', borderRadius: '22px', padding: '26px', boxShadow: '0 12px 30px rgba(15,23,42,0.08)', display: 'flex', flexDirection: 'column', gap: '16px' },
    grid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '16px' }, label: { display: 'flex', flexDirection: 'column', gap: '8px', color: '#334155', fontWeight: 800 }, input: { padding: '12px 14px', border: '1px solid #cbd5e1', borderRadius: '12px', fontSize: '15px', background: '#fff' }, textarea: { minHeight: '90px', padding: '12px 14px', border: '1px solid #cbd5e1', borderRadius: '12px', fontSize: '15px', resize: 'vertical' },
    submitButton: { border: 'none', background: '#2563eb', color: '#fff', padding: '14px 18px', borderRadius: '12px', fontWeight: 800, cursor: 'pointer' }, error: { background: '#fee2e2', color: '#991b1b', borderRadius: '12px', padding: '12px', marginBottom: '16px', fontWeight: 700 }, fieldError: { color: '#dc2626', fontSize: '12px', fontWeight: 600 },
};
