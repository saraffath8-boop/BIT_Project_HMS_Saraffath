...
import { bloodGroupOptions, patientGenderOptions, patientSchema, patientStatusOptions } from '../../schemas/patientSchema';
...
export default function PatientCreatePage() {
    ...
    return <main style={styles.page}><section style={styles.header}><div><p style={styles.kicker}>Patient Management</p><h1 style={styles.title}>Add Patient</h1><p style={styles.subtitle}>Create a new patient record with an automatically generated patient ID.</p></div><Link style={styles.secondaryLink} to="/patients">Back to Patients</Link></section>
        {error && <div style={styles.error}>{error}</div>}
        <form onSubmit={handleSubmit(submit)} style={styles.formCard}><div style={styles.grid}>
            <Field label="Full Name" error={errors.fullName?.message}><input style={styles.input} {...register('fullName')} /></Field>
            <Field label="Date of Birth" error={errors.dateOfBirth?.message}><input style={styles.input} type="date" {...register('dateOfBirth')} /></Field>
            <Field label="Gender" error={errors.gender?.message}><Select {...register('gender')}>{patientGenderOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</Select></Field>
...
const styles = {
    ...
    kicker: { margin: 0, color: '#0e7490', fontWeight: 800, textTransform: 'uppercase' }, title: { margin: '8px 0', fontSize: '34px' }, subtitle: { margin: 0, color: '#64748b' },
    ...
    grid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '16px' }, label: { display: 'flex', flexDirection: 'column', gap: '8px', color: '#334155', fontWeight: 800 },
    input: { padding: '12px 14px', border: '1px solid #cbd5e1', borderRadius: '12px', fontSize: '15px', background: '#fff' }, textarea: { minHeight: '90px', padding: '12px 14px', border: '1px solid #cbd5e1', borderRadius: '12px', fontSize: '15px', resize: 'vertical' },
