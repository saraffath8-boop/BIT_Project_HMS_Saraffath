    ...
    return <main className="space-y-6">
        ...
        <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.25fr)]">
            ...
            <Card><CardHeader><CardTitle>Staff List</CardTitle><CardDescription>Registered staff and administrative accounts.</CardDescription></CardHeader><CardContent>
                ...
                {!loadingUsers && !loadError && staffUsers.length > 0 && <Table><TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Email</TableHead><TableHead>Role</TableHead><TableHead>Status</TableHead><TableHead>Booking Details</TableHead><TableHead>Actions</TableHead></TableRow></TableHeader><TableBody>
                    {staffUsers.map((staffUser) => <TableRow key={staffUser.id || staffUser.email}><TableCell className="font-medium">{staffUser.name || `${staffUser.firstName || ''} ${staffUser.lastName || ''}`.trim()}</TableCell><TableCell>{staffUser.email}</TableCell><TableCell>{formatRole(staffUser.role)}</TableCell><TableCell><Badge variant={staffUser.isActive ? 'success' : 'secondary'}>{staffUser.isActive ? 'Active' : 'Inactive'}</Badge></TableCell>{staffUser.role === 'doctor' ? <DoctorBookingSetup doctor={staffUser} departments={departments} token={token} onSaved={loadStaffUsers} /> : <><TableCell>Not applicable</TableCell><TableCell>Not applicable</TableCell></>}</TableRow>)}
                </TableBody></Table>}
            </CardContent></Card>
        </div>
    </main>;
...
const DoctorBookingSetup = ({ doctor, departments, token, onSaved }) => {
    ...
    if (!editing) {
        return <><TableCell><div className="min-w-52 space-y-1 text-sm">
                <p><span className="font-medium text-slate-900">Department:</span> {doctor.department?.name || 'Not assigned'}</p>
                <p><span className="font-medium text-slate-900">Specialization:</span> {doctor.specialization || 'Not recorded'}</p>
                <p><span className="font-medium text-slate-900">Consultation fee:</span> LKR {Number(doctor.consultationFee || 0).toLocaleString()}</p>
            </div></TableCell>
            <TableCell><Button type="button" size="sm" variant="outline" onClick={() => { resetValues(); setEditing(true); }}><Pencil className="size-4" />Edit</Button></TableCell>
        </>;
    }

    return <><TableCell><div className="min-w-64 space-y-2">
        ...
        <TableCell><div className="flex items-center gap-2">
            <Button type="button" size="sm" onClick={save} disabled={!department || saving}>{saving ? 'Saving...' : 'Save'}</Button>
            <Button type="button" size="sm" variant="outline" onClick={() => { resetValues(); setEditing(false); }} disabled={saving}>Cancel</Button>
        </div></TableCell>
    </>;
};
