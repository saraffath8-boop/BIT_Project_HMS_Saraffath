...
const CellValue = ({ value }) => {
    ...
    return isStatus ? <Badge variant={['completed', 'paid', 'active', 'available', 'approved'].includes(clean.toLowerCase()) ? 'success' : ['cancelled', 'inactive', 'unpaid'].includes(clean.toLowerCase()) ? 'destructive' : 'warning'}>{text}</Badge> : text;
};
        ...
        <main className="space-y-6">
            <section className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
                ...
                <div className="flex flex-wrap gap-2">
                    <Button variant="outline" onClick={loadItems} disabled={loading}><RefreshCw className="size-4" />Refresh</Button>
                    {canCreate && <Button asChild><Link to={createAction.to}><Plus className="size-4" />{createAction.label}</Link></Button>}
                </div>
            </section>

            {error && <Alert variant="destructive">{error}</Alert>}
            {loading && <Card className="space-y-3 p-5">{[1, 2, 3, 4].map((item) => <Skeleton key={item} className="h-11 w-full" />)}</Card>}
            {!loading && !error && items.length === 0 && <Card className="grid place-items-center px-6 py-16 text-center"><SearchX className="mb-3 size-9 text-slate-400" /><h2 className="font-semibold text-slate-800">No records found</h2><p className="mt-1 text-sm text-slate-500">{emptyMessage}</p></Card>}
