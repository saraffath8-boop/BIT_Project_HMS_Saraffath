...
const QueueSection = ({ title, appointments, emptyMessage, action }) => <CollapsibleSection title={title} count={appointments.length}>
    {appointments.length === 0 ? <Card className="p-8 text-center text-sm text-slate-500">{emptyMessage}</Card> : <Card className="overflow-hidden"><Table>
        <TableHeader><TableRow><TableHead>Serial No.</TableHead><TableHead>Patient</TableHead><TableHead>Appointment</TableHead><TableHead>{action ? 'Action' : 'Status'}</TableHead></TableRow></TableHeader>
        <TableBody>{appointments.map((appointment) => <TableRow key={appointment.id}><TableCell className="font-semibold text-cyan-800">{appointment.queueNumber}</TableCell><TableCell><CompactDetail title={getPersonName(appointment.patient)} subtitle={appointment.patient?.phone || 'Phone not recorded'} /></TableCell><TableCell><CompactDetail title={getPersonName(appointment.doctor)} subtitle={formatDateTime(appointment.appointmentDate)} /></TableCell><TableCell>{action ? action(appointment) : <Badge variant="success">{appointment.status === 'paid' ? 'confirmed' : appointment.status.replaceAll('_', ' ')}</Badge>}</TableCell></TableRow>)}</TableBody>
    </Table></Card>}
</CollapsibleSection>;
