...
const PrescriptionsPage = () => {
    ...
    const readyPrescriptions = prescriptions.filter((prescription) => prescription.status !== 'issued' && prescription.status !== 'cancelled');
    const distributedPrescriptions = prescriptions.filter((prescription) => prescription.status === 'issued');

    const renderActions = (prescription) => {
        if (!canIssue) return prescription.paymentStatus === 'paid' ? 'Sent to pharmacy' : 'Awaiting cashier payment';
        ...
        <main style={styles.page}>
            <section style={styles.header}>
                <div>
                    ...
                    <p style={styles.subtitle}>{user?.role === 'pharmacist' ? 'Paid prescriptions ready for distribution and completed distribution history.' : 'View prescriptions created from diagnosis reports and their payment/distribution progress.'}</p>
                </div>
            ...
            {!loading && !error && user?.role === 'pharmacist' && prescriptions.length > 0 && (
                <div style={styles.sections}>
                    <PrescriptionSection
                        ...
                        description="Review every medicine, dosage, frequency, duration, and instruction before handing medicines to the patient."
                        prescriptions={readyPrescriptions}
                        emptyMessage="No paid prescriptions are waiting for distribution."
                        action={(prescription) => (
                            <button type="button" style={styles.primaryButton} disabled={updatingId === prescription.id} onClick={() => handleStatusUpdate(prescription, 'issued')}>
                                {updatingId === prescription.id ? 'Updating...' : 'Mark Distributed'}
                            </button>
                        )}
                    />
                    <PrescriptionSection
                        title="Distribution History"
                        description="Completed distribution records remain saved here with medicine details and distribution information."
                        ...
                        <tbody>
                            {prescriptions.map((prescription) => (
                                <tr key={prescription.id}>
                                    ...
                                    <td style={styles.td}>{prescription.paymentStatus || 'unpaid'}</td>
                                    <td style={styles.td}>{prescription.status === 'issued' ? 'Distributed' : prescription.paymentStatus === 'paid' ? 'Ready for pharmacist' : 'Awaiting cashier payment'}</td>
...
const PrescriptionSection = ({ title, description, prescriptions, emptyMessage, action, history = false }) => (
    <section style={styles.section}>
        <div>
            <h2 style={styles.sectionTitle}>{title} <span style={styles.count}>({prescriptions.length})</span></h2>
        ...
        {prescriptions.map((prescription) => (
            <article key={prescription.id} style={styles.prescriptionCard}>
                <div style={styles.prescriptionHeader}>
                    <div><span style={styles.label}>Patient</span><strong>{getPersonName(prescription.patient)}</strong><p style={styles.meta}>{prescription.patient?.phone || 'Phone not recorded'}</p></div>
                    <div><span style={styles.label}>Doctor</span><strong>{getPersonName(prescription.doctor)}</strong><p style={styles.meta}>Created {formatDateTime(prescription.createdAt)}</p></div>
                    {history && <div><span style={styles.label}>Distributed</span><strong>{formatDateTime(prescription.issuedAt)}</strong><p style={styles.meta}>By {getPersonName(prescription.issuedBy)}</p></div>}
                    {!history && action?.(prescription)}
                </div>
                {prescription.notes && <div style={styles.notes}><strong>Prescription notes:</strong> {prescription.notes}</div>}
                <div style={styles.tableWrap}>
                    <table style={styles.table}>
                        <thead><tr><th style={styles.th}>Medicine</th><th style={styles.th}>Dosage</th><th style={styles.th}>Frequency</th><th style={styles.th}>Duration</th><th style={styles.th}>Instructions</th></tr></thead>
                        <tbody>{(prescription.items || []).map((item) => <tr key={item._id || `${prescription.id}-${item.medicineName}`}><td style={styles.td}><strong>{item.medicineName}</strong></td><td style={styles.td}>{item.dosage}</td><td style={styles.td}>{item.frequency}</td><td style={styles.td}>{item.duration}</td><td style={styles.td}>{item.instructions || 'No special instructions'}</td></tr>)}</tbody>
                    </table>
                </div>
            </article>
        ))}
...
const styles = {
    ...
    sectionDescription: { margin: '5px 0 0', color: '#64748b', lineHeight: 1.5 },
    count: { color: '#64748b', fontSize: '14px', fontWeight: 500 },
    prescriptionCard: { display: 'grid', gap: '16px', border: '1px solid #e2e8f0', borderRadius: '16px', background: '#ffffff', padding: '18px' },
    prescriptionHeader: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '18px', flexWrap: 'wrap' },
    label: { display: 'block', marginBottom: '4px', color: '#64748b', fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em' },
    meta: { margin: '4px 0 0', color: '#64748b', fontSize: '13px' },
    notes: { borderRadius: '10px', background: '#f8fafc', padding: '12px', color: '#334155', fontSize: '14px' },
};
