            ...
            {!loading && !error && notifications.length > 0 && (
                <div style={styles.list}>
                    {notifications.map((notification) => (
                        <article key={notification.id} style={{ ...styles.card, ...(!notification.isRead ? styles.unreadCard : {}) }}>
                            <div>
                                <div style={styles.titleRow}>
                                    <h2 style={styles.sectionTitle}>{notification.title}</h2>
                                    {!notification.isRead && <span style={styles.unreadBadge}>New</span>}
                                </div>
...
const styles = {
    ...
    card: { display: 'flex', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap', background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '16px', padding: '18px' },
    unreadCard: { background: '#ecfeff', border: '2px solid #0891b2', boxShadow: '0 4px 14px rgba(8, 145, 178, 0.12)' },
    titleRow: { display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' },
    unreadBadge: { borderRadius: '999px', background: '#dc2626', color: '#ffffff', padding: '3px 8px', fontSize: '11px', fontWeight: 800, textTransform: 'uppercase' },
