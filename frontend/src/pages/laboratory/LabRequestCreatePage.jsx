...
const LabRequestCreatePage = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        const payload = {
            ...
            tests: [{ testName: formData.testName }],
            priority: formData.priority,
        };
        if (user?.role !== 'doctor') payload.doctor = formData.doctor;
        try {
            await createLabRequest(payload, token);
            setSuccess('Lab request created successfully.');
            navigate('/laboratory');
        } catch (err) {
            setError(err.message || 'Unable to create lab request');
        } finally {
            setSubmitting(false);
        }
    };
    return (
        <main style={styles.page}>
            <section style={styles.header}>
                <div>
                    <p style={styles.kicker}>Laboratory Management</p>
                    <h1 style={styles.title}>Create Lab Request</h1>
                    <p style={styles.subtitle}>Request a laboratory test for a selected patient.</p>
                </div>
                <div style={styles.actions}>
                    <Link style={styles.secondaryLink} to="/laboratory">
                        Laboratory
                    </Link>
            ...
            <form style={styles.form} onSubmit={handleSubmit}>
                ...
                <button style={styles.primaryButton} type="submit" disabled={submitting}>
                    {submitting ? 'Creating Lab Request...' : 'Create Lab Request'}
                </button>
            </form>
        </main>
    );
