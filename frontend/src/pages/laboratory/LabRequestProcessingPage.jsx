...
export default function LabRequestProcessingPage() {
    ...
    if (loading)
        return (
            <main>
                <Card className="p-6 text-sm text-slate-500">Loading laboratory request...</Card>
            </main>
        );
    return (
        <main className="space-y-6">
            <section className="flex flex-wrap items-start justify-between gap-4">
                <div>
                    <p className="page-kicker">Laboratory Processing</p>
                    <h1 className="page-title">
                        {request?.patient?.fullName || 'Laboratory Request'}
                    </h1>
                    <p className="page-description">
                        Results remain linked to the patient and consultation medical record.
                    </p>
                </div>
                <Button asChild variant="outline">
                    <Link to="/laboratory">Laboratory Requests</Link>
                </Button>
            </section>
                ...
                <form onSubmit={save} className="space-y-6">
                    <Card>
                        <CardHeader>
                            <div className="flex flex-wrap justify-between gap-3">
                                <div>
                                    ...
                                    <CardDescription>
                                        Doctor: {request.doctor?.name || 'Not recorded'} | Priority:{' '}
                                        {request.priority}
                                    </CardDescription>
                                </div>
                        ...
                        <CardContent>
                            ...
                            <select
                                disabled={!canProcess}
                                className="mt-2 h-10 w-full max-w-xs rounded-lg border border-slate-200 bg-white px-3 text-sm"
                                value={status}
                            ...
                            {tests.map((test, index) => (
                                ...
                                >
                                    <Field label="Test Name">
                                        <Input disabled value={test.testName} />
                                    </Field>
                                    <Field label="Result">
                                        <Input
                                            disabled={!canProcess}
                                            value={test.result || ''}
                                            onChange={(event) =>
                                                updateTest(index, 'result', event.target.value)
                                            }
                                        />
                                    </Field>
                                    ...
                                    <Field label="Remarks">
                                        <Input
                                            ...
                                            value={test.remarks || ''}
                                            onChange={(event) =>
                                                updateTest(index, 'remarks', event.target.value)
                                            }
                                        />
                                    </Field>
                                </div>
                    ...
                    {canProcess && (
                        <Button type="submit" disabled={saving}>
                            {saving ? 'Saving results...' : 'Save Laboratory Results'}
                        </Button>
                    )}
                </form>
            )}
