...
export default function LabRequestProcessingPage() {
    ...
    const save = async (event) => {
        ...
        try {
            ...
            setRequest(response.labRequest); setSuccess(status === 'completed' ? 'Laboratory report completed. Patient, doctor, and receptionist were notified.' : 'Laboratory request updated successfully.');
    ...
    return <main className="space-y-6">
        ...
        {request && <form onSubmit={save} className="space-y-6">
            <Card><CardHeader><div className="flex flex-wrap justify-between gap-3"><div><CardTitle>Request Summary</CardTitle><CardDescription>Doctor: {request.doctor?.name || 'Not recorded'} · Priority: {request.priority}</CardDescription></div><Badge variant="success">{request.patientDecisionStatus?.replaceAll('_', ' ')}</Badge></div></CardHeader><CardContent><Label>Status</Label><select disabled={!canProcess} className="mt-2 h-10 w-full max-w-xs rounded-lg border border-slate-200 bg-white px-3 text-sm" value={status} onChange={(event) => setStatus(event.target.value)}><option value="requested">Requested</option><option value="sample_collected">Sample Collected</option><option value="in_progress">In Progress</option><option value="completed">Completed</option><option value="cancelled">Cancelled</option></select></CardContent></Card>
