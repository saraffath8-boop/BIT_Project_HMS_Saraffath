...
import { calculateAge, normalizeNic, PERSON_NAME_REGEX, SRI_LANKAN_NIC_REGEX, SRI_LANKAN_PHONE_REGEX } from '../utils/userValidation.js';
...
const userSchema = new mongoose.Schema(
    {
        ...
        firstName: { type: String, required: [true, 'First name is required'], trim: true, minlength: [2, 'First name must be at least 2 characters long'], match: [PERSON_NAME_REGEX, 'First name contains invalid characters'] },
        lastName: { type: String, required: [true, 'Last name is required'], trim: true, minlength: [2, 'Last name must be at least 2 characters long'], match: [PERSON_NAME_REGEX, 'Last name contains invalid characters'] },
        ...
        phone: { type: String, required: [true, 'Phone number is required'], unique: true, sparse: true, trim: true, match: [SRI_LANKAN_PHONE_REGEX, 'Phone number must be a valid Sri Lankan 10-digit number starting with 0'] },
        nic: { type: String, required: [true, 'NIC is required'], unique: true, sparse: true, trim: true, set: normalizeNic, match: [SRI_LANKAN_NIC_REGEX, 'NIC must be 12 digits or 9 digits followed by V/X'] },
        dob: {
            type: Date,
            required: [true, 'Date of birth is required'],
            validate: {
                validator(value) {
                    return value <= new Date() && (this.role === USER_ROLE_VALUES.PATIENT || calculateAge(value) >= 18);
                },
                message: 'Date of birth must not be in the future and staff users must be at least 18 years old',
            },
        },
