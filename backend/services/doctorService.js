...
export const buildAppointmentDate = (dateValue, timeSlot) => {
    ...
    if (Number.isNaN(appointmentDate.getTime())) throw serviceError('Selected time slot is invalid');
...
export const getDoctorOrThrow = async (doctorId) => {
    ...
    if (!doctor || doctor.role !== 'doctor' || !doctor.isActive) throw serviceError('Doctor not found', 404);
    return doctor;
};
...
const sanitizeDoctor = (doctor) => ({
    ...
    department: doctor.department ? { id: doctor.department._id.toString(), name: doctor.department.name } : null,
    specialization: doctor.specialization || '',
    consultationFee: doctor.consultationFee || 0,
    availableDays: doctor.availableDays?.length ? doctor.availableDays : DEFAULT_DAYS,
    availableTimeSlots: doctor.availableTimeSlots?.length ? doctor.availableTimeSlots : DEFAULT_SLOTS,
});
...
const getDoctorAvailability = async (doctorId, dateValue) => {
    ...
    const scheduledSlots = doctor.availableTimeSlots?.length ? doctor.availableTimeSlots : DEFAULT_SLOTS;

    if (!availableDays.includes(selectedDate.getDay())) {
        return { doctor: sanitizeDoctor(doctor), date: dateValue, slots: [] };
    }

    const slotChecks = await Promise.all(scheduledSlots.map(async (timeSlot) => {
