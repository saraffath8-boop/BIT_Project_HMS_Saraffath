...
export const validateUserCreateInput = ({
    ...
    if (
        [firstName, lastName, email, phone, nic, gender, password].some(
    ...
    if (cleanFirstName.length < 2 || !PERSON_NAME_REGEX.test(cleanFirstName))
        return 'First name must be at least 2 characters and contain valid name characters';
    if (cleanLastName.length < 2 || !PERSON_NAME_REGEX.test(cleanLastName))
        return 'Last name must be at least 2 characters and contain valid name characters';
    if (!/^\S+@\S+\.\S+$/.test(email.trim())) return 'Please provide a valid email address';
    if (!SRI_LANKAN_PHONE_REGEX.test(phone.trim()))
        return 'Phone number must be a valid Sri Lankan 10-digit number starting with 0';
    if (!SRI_LANKAN_NIC_REGEX.test(nic.trim()))
    ...
    if (role !== PATIENT_ROLE && calculateAge(birthDate) < 18)
        return 'Staff users must be at least 18 years old';
