...
export const signupUser = async (req, res) => {
    try {
        const { firstName, lastName, email, phone, nic, dob, gender, password, address, emergencyContactName, emergencyContactPhone } = req.body;
        ...
        const result = await authService.signupUser({ firstName, lastName, email, phone, nic, dob, gender, password, address, emergencyContactName, emergencyContactPhone });
...
export const requestPatientPasswordReset = async (req, res) => {
    try {
        ...
        if (!isValidMobile(phone)) return sendError(res, 400, 'Enter a valid Sri Lankan 10-digit mobile number starting with 07');
...
export const resetPatientPassword = async (req, res) => {
    try {
        ...
        if (!isValidMobile(phone)) return sendError(res, 400, 'Enter a valid Sri Lankan 10-digit mobile number starting with 07');
        if (!/^[0-9]{6}$/.test(String(otp || ''))) return sendError(res, 400, 'OTP must contain exactly 6 digits');
        if (typeof newPassword !== 'string' || newPassword.length < 8) return sendError(res, 400, 'Password must be at least 8 characters long');
